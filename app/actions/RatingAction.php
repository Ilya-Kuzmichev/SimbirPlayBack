...
class RatingAction extends Action
{
    public function list(Request $request, Response $response, $args)
    {
        ...
        $users = $this->db::select("SELECT u.id, u.name, u.surname, u.patronymic, u.department_id as departmentId, SUM(b.bonus) rating FROM {$tableUser} u INNER JOIN {$tableBonus} b ON u.id = b.user_id {$where} GROUP BY u.id ORDER BY rating DESC");
