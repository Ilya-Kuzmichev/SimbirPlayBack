...
class RatingAction extends Action
{
    public function list(Request $request, Response $response, $args)
    {
        ...
        $users = $this->container['db']::select("SELECT u.id, u.name, u.surname, u.department_id, SUM(b.bonus) rating FROM {$tableUser} u INNER JOIN {$tableBonus} b ON u.id = b.user_id {$where} GROUP BY u.id ORDER BY rating DESC");
